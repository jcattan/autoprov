...
if(getMethod() == "GET") {
   ...
   // if(preg_match('/[0-9A-Fa-f]{12}/i', $strip, $matches) && !(preg_match('/[0]{10}[0-9]{2}/i',$strip))) {
        ...
        $sql = "SELECT id FROM `autoprov_mac_list` WHERE `mac` LIKE '%" . $mac_address . "%'";
        $mac_id = sql($sql, 'getOne');
        $phone_info = FreePBX::Autoprov()->get_phone_info($mac_id);
		$files = FreePBX::Autoprov()->prepare_configs($phone_info, FALSE, FALSE);
