...
class Autoprov_Config
{
	...
	public function epm_config_hardware_get_list_models($id_product=NULL, $show_all = true, $byorder = "model")
	{
		...
		if($show_all == true) 	{ $sql = 'SELECT * FROM autoprov_model_list WHERE product_id = '.$id_product.' ORDER BY '.$byorder.' ASC'; }
		else 					{ $sql = 'SELECT * FROM autoprov_model_list WHERE hidden = 0 AND product_id = '.$id_product.' ORDER BY '.$byorder.' ASC'; }
	...
	public function epm_config_hardware_get_list_product($id_brand=NULL, $show_all = true, $byorder = "long_name")
	{
		...
		if ($show_all == true) 	{ $sql = 'SELECT * FROM autoprov_product_list WHERE brand = '.$id_brand.' ORDER BY '.$byorder.' ASC'; }
		else 					{ $sql = 'SELECT * FROM autoprov_product_list WHERE hidden = 0 AND brand = '.$id_brand.' ORDER BY '.$byorder.' ASC'; }
		$result = sql($sql, 'getAll', DB_FETCHMODE_ASSOC);
		return $result;
	}
	...
	public function epm_config_hardware_get_list_brand($show_all = true, $byorder = "id") {
		if ($show_all == true) 	{ $sql = "SELECT * from autoprov_brand_list WHERE id > 0 ORDER BY " . $byorder . " ASC "; }
		else 					{ $sql = "SELECT * from autoprov_brand_list WHERE id > 0 AND hidden = 0 ORDER BY " . $byorder . " ASC "; }
		$result = sql($sql, 'getAll', DB_FETCHMODE_ASSOC);
		return $result;
	}
	...
	*/
	...
	private function epm_config_manager_saveconfig()
	{
		...
		if ($dget['typesavecfg'] == "enabled") {
			...
			switch($dget['idtype']) {
				case "marca":
					$sql = "UPDATE autoprov_brand_list SET hidden = '".$dget['value'] ."' WHERE id = '".$dget['id']."'";
					break;

				case "producto":
					$sql = "UPDATE autoprov_product_list SET hidden = '". $dget['value'] ."' WHERE id = '".$dget['id']."'";
					break;

				case "modelo":
					$sql = "UPDATE autoprov_model_list SET hidden = '". $dget['value'] ."' WHERE id = '".$dget['id']."'";
					break;
...
public function epm_config_manager_hardware_get_list_all_hide_show(): array
{
	...
	//http://pbx.cerebelum.lan/admin/ajax.php?module=autoprov&module_sec=epm_config&module_tab=manager&command=list_all_brand
	public function epm_config_manager_hardware_get_list_all()
	{
		$row_out = array();
		$i = 0;
		$brand_list = $this->epm_config_hardware_get_list_brand(true, "name");
		//FIX: https://github.com/FreePBX-ContributedModules/autoprov/commit/2ad929d0b38f05c9da1b847426a4094c3314be3b
	...
	function brand_update_check($brand_name_find = NULL)
	{
		...
		$sql = "SELECT * FROM  autoprov_brand_list WHERE directory = '" . $brand_name_find . "'";
	...
	function brand_update_check_all()
	{
		...
		$sql = 'SELECT * FROM  autoprov_brand_list WHERE id > 0';
    ...
    function update_check($echomsg = false, &$error=array()) {
        ...
        if (!$this->configmod->get('use_repo')) {
            ...
            $sql = "SELECT value FROM autoprov_global_vars WHERE var_name LIKE 'endpoint_vers'";
            $data = sql($sql, 'getOne');
            ...
            if ($contents != '1') {
                if (($data == "") OR ($data <= $endpoint_last_mod)) {
                    if ((!$master_result) OR (!$this->system->download_file($this->UPDATE_PATH . '/' . $endpoint_package, $temp_location . $endpoint_package)))
                    {
                        ...
                        $sql = "UPDATE autoprov_global_vars SET value = '" . $endpoint_last_mod . "' WHERE var_name = 'endpoint_vers'";
                        sql($sql);
                    }
                }
                ...
                if ($master_result) {
                    ...
                    foreach ($out as $data) {
                        ...
                        $local = sql("SELECT local FROM autoprov_brand_list WHERE  directory =  '" . $data['directory'] . "'", 'getOne');
                        if (!$local) {
                        	if ($echomsg == true) {
                        ...
                        if (file_exists($this->PHONE_MODULES_PATH . "endpoint/" . $data['directory'] . "/brand_data.json")) {
							...
							if (array_key_exists('oui_list', $temp)) {
                            	foreach ($temp['oui_list'] as $oui) {
									$sql = "REPLACE INTO autoprov_oui_list (`oui`, `brand`, `custom`) VALUES ('" . $oui . "', '" . $temp['brand_id'] . "', '0')";
                               		sql($sql);
								}
							}
                            $brand_name = $temp['directory'];
							...
							{
								$sql = 'SELECT directory FROM autoprov_brand_list where id = "'.$temp['brand_id'].'"';
								...
								if ($datoif != "") {
                                	...
                                	$sql = "INSERT INTO autoprov_brand_list (id, name, directory, cfg_ver) VALUES ('" . $temp['brand_id'] . "', '" . $temp['name'] . "', '" . $temp['directory'] . "', '" . $version[$brand_name] . "')";
                                	sql($sql);
								}
                            } else {
                                //in database already!
            ...
            $out = $temp['data']['brands'];
            $row = sql('SELECT * FROM  autoprov_brand_list WHERE id > 0', 'getAll', DB_FETCHMODE_ASSOC);
            ...
            foreach ($out as $data) {
                ...
                if (array_key_exists('directory', $temp['data']['brands'])) {
                    ...
                    $b_data = sql("SELECT id FROM autoprov_brand_list WHERE id = '" . $brand_id . "'", 'getOne');
                    if ($b_data) {
                        $sql = "UPDATE autoprov_brand_list SET local = '1', name = '" . $brand_name . "', cfg_ver = '" . $brand_version . "', installed = 1, hidden = 0 WHERE id = " . $brand_id;
                        sql($sql);
                    } else {
                        $sql = "INSERT INTO autoprov_brand_list (id, name, directory, cfg_ver, local, installed) VALUES ('" . $brand_id . "', '" . $brand_name . "', '" . $directory . "', '" . $brand_version . "', '1', '1')";
                        sql($sql);
                    }
                    ...
                    foreach ($temp['data']['brands']['family_list'] as $family_list) {
                        ...
                        foreach ($family_line_xml['data']['model_list'] as $model_list) {
                            ...
                            $m_data = sql("SELECT id FROM autoprov_model_list WHERE id='" . $brand_id . $family_line_xml['data']['id'] . $model_list['id'] . "'", 'getOne');
                            if ($m_data) {
                                $sql = "UPDATE autoprov_model_list SET max_lines = '" . $model_list['lines'] . "', model = '" . $model_list['model'] . "', template_list = '" . $template_list . "' WHERE id = '" . $brand_id . $family_line_xml['data']['id'] . $model_list['id'] . "'";
                            } else {
                                $sql = "INSERT INTO autoprov_model_list (`id`, `brand`, `model`, `max_lines`, `product_id`, `template_list`, `enabled`, `hidden`) VALUES ('" . $brand_id . $family_line_xml['data']['id'] . $model_list['id'] . "', '" . $brand_id . "', '" . $model_list['model'] . "', '" . $model_list['lines'] . "', '" . $brand_id . $family_line_xml['data']['id'] . "', '" . $template_list . "', '0', '0')";
                            }
                            sql($sql);
                        ...
                        $sql = "SELECT * FROM autoprov_model_list WHERE product_id = " . $family_id;
                        ...
                        foreach ($products as $data) {
							if (!$this->system->arraysearchrecursive($data['model'], $family_line_xml['data']['model_list'], 'model')) {
                                ...
                                $sql = 'DELETE FROM autoprov_model_list WHERE id = ' . $data['id'];
                                sql($sql);
                                $sql = "SELECT id FROM autoprov_model_list WHERE model LIKE '" . $model_name . "'";
                                $new_model_id = sql($sql, 'getOne');
                                if ($new_model_id) {
                                    $sql = "UPDATE  autoprov_mac_list SET  model =  '" . $new_model_id . "' WHERE  model = '" . $data['id'] . "'";
                                } else {
                                    $sql = "UPDATE  autoprov_mac_list SET  model =  '0' WHERE  model = '" . $data['id'] . "'";
                                }
                                sql($sql);
                                out (_("Done!"));
                            }
                    ...
                    foreach ($temp['data']['brands']['oui_list'] as $oui) {
                        $sql = "REPLACE INTO autoprov_oui_list (`oui`, `brand`, `custom`) VALUES ('" . $oui . "', '" . $brand_id . "', '0')";
                        sql($sql);
                    }
                }
            }
    ...
    function sync_model($model, &$error = array()) {
        if ((!empty($model)) OR ($model > 0)) {
            $sql = "SELECT * FROM  autoprov_model_list WHERE id='" . $model . "'";
            $model_row = sql($sql, 'getrow', DB_FETCHMODE_ASSOC);

            $sql = "SELECT * FROM  autoprov_product_list WHERE id='" . $model_row['product_id'] . "'";
            $product_row = sql($sql, 'getRow', DB_FETCHMODE_ASSOC);

            $sql = "SELECT * FROM  autoprov_brand_list WHERE id=" . $model_row['brand'];
            ...
            $sql = "UPDATE autoprov_model_list SET max_lines = '" . $maxlines . "', template_list = '" . $template_list . "' WHERE id = '" . $model . "'";
            sql($sql);
            ...
            $sql = "UPDATE autoprov_product_list SET long_name = '" . str_replace("'", "''", $long_name) . "', short_name = '" . str_replace("'", "''", $short_name) . "' , cfg_ver = '" . $version . "' WHERE id = '" . $product_row['id'] . "'";
            sql($sql);
            ...
            $sql = "UPDATE autoprov_model_list SET template_data = '" . serialize($template_data_array) . "' WHERE id = '" . $model . "'";
            sql($sql);
            return(TRUE);
        } else {
            return(FALSE);
    ...
    function download_brand($id) {
        ...
        if (!$this->configmod->get('use_repo')) {
            ...
            $row = sql('SELECT * FROM  autoprov_brand_list WHERE id =' . $id, 'getAll', DB_FETCHMODE_ASSOC);
...
//echo ".";
                ...
                if ($b_data) {
                    ...
                    $sql = "UPDATE autoprov_brand_list SET local = '" . $local . "', name = '" . $brand_name . "', cfg_ver = '" . $brand_version . "', installed = 1, hidden = 0 WHERE id = " . $brand_id;
                    sql($sql);
                    out(_("Done!"));
                } else {
                	outn(sprintf(_("Inserting %s brand data ..."), $brand_name));
					$sql = "INSERT INTO autoprov_brand_list (id, name, directory, cfg_ver, local, installed) VALUES ('" . $brand_id . "', '" . $brand_name . "', '" . $directory . "', '" . $brand_version . "', '" . $local . "', '1')";
                    sql($sql);
                    out(_("Done!"));
                }
                ...
                foreach ($temp['data']['brands']['family_list'] as $family_list) {
					...
					if (count((array)$family_line_xml['data']['model_list']) > 0) {
	                    ...
	                    foreach ($family_line_xml['data']['model_list'] as $model_list) {
	                        ...
	                        $sql = 'SELECT id, global_custom_cfg_data, global_user_cfg_data FROM autoprov_mac_list WHERE model = ' . $model_final_id;
	                        $old_data = NULL;
...
outn(_("----Old Data Detected! Migrating ... "));
	                                ...
	                                $sql = "UPDATE autoprov_mac_list SET  global_custom_cfg_data =  '" . $final_data . "' WHERE  id =" . $data['id'];
	                                sql($sql);
									out(_("Done!"));
	                            }
...
outn(_("Old Data Detected! Migrating ... "));
	                                ...
	                                $sql = "UPDATE autoprov_mac_list SET  global_user_cfg_data =  '" . $final_data . "' WHERE  id =" . $data['id'];
	                                sql($sql);
									out(_("Done!"));
	                            }
	                        }
	                        $old_data = NULL;
	                        $sql = 'SELECT id, global_custom_cfg_data FROM autoprov_template_list WHERE model_id = ' . $model_final_id;
	                        $old_data = sql($sql, 'getAll', DB_FETCHMODE_ASSOC);
	                        foreach ($old_data as $data) {
	                            $global_custom_cfg_data = unserialize($data['global_custom_cfg_data']);
	                            if ((is_array($global_custom_cfg_data)) AND (!array_key_exists('data', $global_custom_cfg_data))) {
out(_("Old Data Detected! Migrating ... "));
	                                ...
	                                $final_data['ari'] = $new_ari;
	                                $final_data = serialize($final_data);
	                                $sql = "UPDATE autoprov_template_list SET  global_custom_cfg_data =  '" . $final_data . "' WHERE  id =" . $data['id'];
	                                sql($sql);
									out(_("Done!"));
	                            }
	                        }
...
if ($this->configmod->get('debug')) echo format_txt(_("---Inserting Model %_NAMEMOD_%"), "", array("%_NAMEMOD_%" => $model_list['model']));
    ...
    function remove_brand($id=NULL, $remove_configs=FALSE, $force=FALSE) {
        ...
        if (!$this->configmod->get('use_repo')) {
            $sql = "SELECT id, firmware_vers FROM autoprov_product_list WHERE brand = '" . $id . "'";
            $products = sql($sql, 'getall', DB_FETCHMODE_ASSOC);
			...
			$sql = "SELECT directory FROM autoprov_brand_list WHERE id = '" . $id . "'";
            $brand_dir = sql($sql, 'getOne');
            $this->system->rmrf($this->PHONE_MODULES_PATH . "endpoint/" . $brand_dir);

            $sql = "DELETE FROM autoprov_model_list WHERE brand = '" . $id . "'";
            sql($sql);

            $sql = "DELETE FROM autoprov_product_list WHERE brand = '" . $id . "'";
            sql($sql);

            $sql = "DELETE FROM autoprov_oui_list WHERE brand = '" . $id . "'";
            sql($sql);

            $this->system->rmrf($this->PHONE_MODULES_PATH . $brand_dir);
            $sql = "DELETE FROM autoprov_brand_list WHERE id = '" . $id . "'";
            sql($sql);
    ...
    function install_firmware($product_id) {
        ...
        $temp_directory = $this->system->sys_get_temp_dir() . "/epm_temp/";
        $sql = 'SELECT autoprov_product_list.*, autoprov_brand_list.directory FROM autoprov_product_list, autoprov_brand_list WHERE autoprov_product_list.brand = autoprov_brand_list.id AND autoprov_product_list.id = ' . $product_id;
        $row = sql($sql, 'getRow', DB_FETCHMODE_ASSOC);
        ...
        if ($json_data['data']['firmware_ver'] > $row['firmware_vers']) {
            ...
            if ($md5_xml == $md5_pkg) {
                ...
                $list = implode(",", $list);
                $sql = "UPDATE autoprov_product_list SET firmware_vers = '" . $json_data['data']['firmware_ver'] . "', firmware_files = '" . $list . "' WHERE id = " . $row['id'];
                sql($sql);
    ...
    function remove_firmware($id) {
        ...
        $sql = "SELECT firmware_files FROM  autoprov_product_list WHERE  id ='" . $id . "'";
        ...
        $sql = "UPDATE autoprov_product_list SET firmware_files = '', firmware_vers = '' WHERE id = '" . $id . "'";
        sql($sql);
    ...
    function firmware_update_check($id=NULL) {
        $sql = "SELECT * FROM  autoprov_product_list WHERE  id ='" . $id . "'";
        $row = sql($sql, 'getRow', DB_FETCHMODE_ASSOC);

        $sql = "SELECT directory FROM  autoprov_brand_list WHERE id ='" . $row['brand'] . "'";
    ...
    function firmware_local_check($id=NULL) {
        $sql = "SELECT * FROM  autoprov_product_list WHERE hidden = 0 AND id ='" . $id . "'";
        $res = sql($sql, 'getAll', DB_FETCHMODE_ASSOC);

        if (count($res) > 0) {
            $row = sql($sql, 'getRow', DB_FETCHMODE_ASSOC);

            $sql = "SELECT directory FROM  autoprov_brand_list WHERE hidden = 0 AND id ='" . $row['brand'] . "'";
            $brand_directory = sql($sql, 'getOne');
