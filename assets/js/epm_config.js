...
function epm_config_document_ready () {
	...
	$('#epm_config_manager_select_hidens').on('changed.bs.select', function (e, clickedIndex, newValue, oldValue) {
		...
		var url = "ajax.php?module=autoprov&module_sec=epm_config&command=saveconfig&typesavecfg=hidden&idtype=" + type_send + "&value=0&idbt=" + id;
...
function epm_config_manager_ajax_hide (e) 
{
	...
	var url = "ajax.php?module=autoprov&module_sec=epm_config&command=saveconfig&typesavecfg=hidden&idtype=" + idtype + "&value=1&idbt=" + id;
	$.getJSON(url, function(data)
	{
		if (data.status == true) {
