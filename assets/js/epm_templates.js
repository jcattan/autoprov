...
function epm_templates_document_ready () {
	...
	var arrayJs = ['assets/autoprov/js/addon/simplescrollbars.js', 'assets/autoprov/js/mode/xml.js'];
...
function epm_template_edit_select_area_list (obj)
{
	...
	if (silent_mode == true) 
	{
		...
		<input name="custom" id="custom" type="hidden" value="<?php echo $dtemplate['hidden_custom'] ; ?>">
			...
			$sql = "UPDATE  autoprov_mac_list SET  model =  '".$_REQUEST['model_list']."' WHERE  id =".$_REQUEST['edit_id']; -> id cambiar por template_id
...
function epm_templates_grid_clone()
{
	...
	/*
		//edit
		//<a href="#" onclick="return popitup('config.php?type=tool&display=epm_config&amp;quietmode=1&amp;handler=file&amp;file=popup.html.php&amp;module=autoprov&amp;pop_type=alt_cfg_edit', '<?php echo $row['name']; ?>')">
		...
		//edit
		//<a href='#' onclick='return popitup2("config.php?type=tool&display=epm_config&amp;quietmode=1&amp;handler=file&amp;file=popup.html.php&amp;module=autoprov&amp;pop_type=alt_cfg_edit", "<?php echo $row['name']?>")'>
