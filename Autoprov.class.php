...
class Autoprov implements \BMO {
	...
	function tftp_check() {
        ...
        $sql = 'SELECT value FROM autoprov_global_vars WHERE var_name = \'tftp_check\'';
        if (sql($sql, 'getOne') != 1) {
            $sql = 'UPDATE autoprov_global_vars SET value = \'1\' WHERE var_name = \'tftp_check\'';
            sql($sql);
...
echo 'TFTP Server is not running. <br />See here for instructions on how to install one: <a href="http://wiki.provisioner.net/index.php/Tftp" target="_blank">http://wiki.provisioner.net/index.php/Tftp</a>';
            ...
            $sql = 'UPDATE autoprov_global_vars SET value = \'0\' WHERE var_name = \'tftp_check\'';
            sql($sql);
        } else {
...
//$this->error['add_device'] = 
			out(_("You Must Select an Extension/Device From the Drop Down") . "!");
    			...
    			if ($this->epm_config->sync_model($model)) {
    				$sql = "SELECT id,template_id FROM autoprov_mac_list WHERE mac = '" . $mac . "'";
    				...
    				if ($dup) {
    					...
    					$sql = "UPDATE autoprov_mac_list SET model = " . $model . ", template_id =  " . $template . " WHERE id = " . $dup['id'];
    					sql($sql);
    					...
    					$sql = "SELECT mac_id FROM autoprov_line_list WHERE ext = " . $ext;
    					$used = sql($sql, 'getOne');
...
//$this->error['add_device'] = 
    					...
    					$sql = 'SELECT autoprov_product_list. * , autoprov_model_list.template_data, autoprov_brand_list.directory FROM autoprov_model_list, autoprov_brand_list, autoprov_product_list WHERE autoprov_model_list.id =  \'' . $model . '\' AND autoprov_model_list.brand = autoprov_brand_list.id AND autoprov_model_list.product_id = autoprov_product_list.id';
    					$row = sql($sql, 'getRow', DB_FETCHMODE_ASSOC);
    
    					$sql = "INSERT INTO `autoprov_mac_list` (`mac`, `model`, `template_id`) VALUES ('" . $mac . "', '" . $model . "', '" . $template . "')";
    					sql($sql);
    					...
    					$sql = "INSERT INTO `autoprov_line_list` (`mac_id`, `ext`, `line`, `description`) VALUES ('" . $ext_id . "', '" . $ext . "', '" . $line . "', '" . addslashes($name) . "')";
    					sql($sql);
    	...
    	if ((!isset($line)) AND (!isset($ext))) {
    		if ($this->linesAvailable(NULL, $mac_id)) {
    			if ($this->eda->all_unused_registrations()) {
    				$sql = 'SELECT * FROM autoprov_line_list WHERE mac_id = ' . $mac_id;
    				...
    				foreach ($lines_list as $row) {
    					...
    					$sql = "UPDATE autoprov_line_list SET line = '" . $row['line'] . "', ext = '" . $row['ext'] . "', description = '" . $this->eda->escapeSimple($name) . "' WHERE luid =  " . $row['luid'];
    					sql($sql);
    				}
    				...
    				$sql = "INSERT INTO `autoprov_line_list` (`mac_id`, `ext`, `line`, `description`) VALUES ('" . $mac_id . "', '" . $reg[0]['value'] . "', '" . $lines[0]['value'] . "', '" . addslashes($name) . "')";
    				sql($sql);
...
//$this->error['add_line'] =
    		...
    		if ($this->linesAvailable(NULL, $mac_id)) {
    			if ($this->eda->all_unused_registrations()) {
    				...
    				$sql = "INSERT INTO `autoprov_line_list` (`mac_id`, `ext`, `line`, `description`) VALUES ('" . $mac_id . "', '" . $ext . "', '" . $lines[0]['value'] . "', '" . addslashes($displayname) . "')";
    				sql($sql);
...
//$this->error['add_line'] =
    		...
    		$sql = "SELECT luid FROM autoprov_line_list WHERE line = '" . $line . "' AND mac_id = " . $mac_id;
    		$luid = sql($sql, 'getOne');
    		if ($luid) {
//$this->error['add_line'] =
			out(_("This line has already been assigned!"));
    			...
    			$sql = "INSERT INTO `autoprov_line_list` (`mac_id`, `ext`, `line`, `description`) VALUES ('" . $mac_id . "', '" . $ext . "', '" . $line . "', '" . addslashes($name) . "')";
    			sql($sql);
//$this->message['add_line'] =
    ...
    function linesAvailable($lineid=NULL, $macid=NULL) {
    	if (isset($lineid)) {
    		$sql = "SELECT max_lines FROM autoprov_model_list WHERE id = (SELECT autoprov_mac_list.model FROM autoprov_mac_list, autoprov_line_list WHERE autoprov_line_list.luid = " . $lineid . " AND autoprov_line_list.mac_id = autoprov_mac_list.id)";
    
    		$sql_l = "SELECT line, mac_id FROM `autoprov_line_list` WHERE luid = " . $lineid;
    		$line = sql($sql_l, 'getRow', DB_FETCHMODE_ASSOC);
    
    		$sql_lu = "SELECT line FROM autoprov_line_list WHERE mac_id = " . $line['mac_id'];
    	} elseif (isset($macid)) {
    		$sql = "SELECT max_lines FROM autoprov_model_list WHERE id = (SELECT model FROM autoprov_mac_list WHERE id =" . $macid . ")";
    		$sql_lu = "SELECT line FROM autoprov_line_list WHERE mac_id = " . $macid;
     ...
     * @param integer $mac_id ID number reference from the MySQL database referencing the table autoprov_mac_list
     * @return array
     * @example
...
//$this->error['get_phone_info'] =
    	...
    	$sql = "SELECT id FROM autoprov_mac_list WHERE model > 0 AND id =" . $mac_id;
...
//$this->error['get_phone_info'] =
    ...
    function get_brand_from_mac($mac) {
    	...
    	$oui_sql = "SELECT autoprov_brand_list.name, autoprov_brand_list.id FROM autoprov_oui_list, autoprov_brand_list WHERE oui LIKE '%" . $oui . "%' AND autoprov_brand_list.id = autoprov_oui_list.brand AND autoprov_brand_list.installed = 1 LIMIT 1";
...
$this->error['parse_configs'] = "File not written to hard drive!";
    ...
    */
    ...
    function discover_new($netmask, $use_nmap=TRUE) {
        ...
        foreach ($temp as $key => &$value) {
            ...
            if ($mac_strip != "000000000000") {
                ...
                $epm_sql = "SELECT * FROM autoprov_mac_list WHERE mac LIKE  '%" . $mac_strip . "%'";
    ...
    function display_templates($product_id, $temp_select = NULL) {
        $i = 0;
        $sql = "SELECT id FROM  autoprov_product_list WHERE autoprov_product_list.id ='" . $product_id . "'";
        $id = sql($sql, 'getOne');

        $sql = "SELECT * FROM  autoprov_template_list WHERE  product_id = '" . $id . "'";
    ...
    function update_line($luid=NULL, $macid=NULL, $name=NULL, $line=NULL) {
        if (isset($luid)) {
            $sql = "SELECT * FROM autoprov_line_list WHERE luid = " . $luid;
            ...
            $sql = "UPDATE autoprov_line_list SET line = '" . $line . "', ext = '" . $row['ext'] . "', description = '" . $this->eda->escapeSimple($name) . "' WHERE luid =  " . $row['luid'];
            sql($sql);
            return(TRUE);
        } else {
            $sql = "SELECT * FROM autoprov_line_list WHERE mac_id = " . $macid;
            $lines_info = sql($sql, 'getAll', DB_FETCHMODE_ASSOC);
    ...
    function delete_line($lineid, $allow_device_remove=FALSE) {
        $sql = 'SELECT mac_id FROM autoprov_line_list WHERE luid = ' . $lineid;
        $mac_id = sql($sql, 'getOne');
        $row = $this->get_phone_info($mac_id);

        $sql = 'SELECT COUNT(*) FROM autoprov_line_list WHERE mac_id = ' . $mac_id;
        $num_lines = sql($sql, 'getOne');
        if ($num_lines > 1) {
            $sql = "DELETE FROM autoprov_line_list WHERE luid=" . $lineid;
            sql($sql);
            ...
            if ($allow_device_remove) {
                ...
                $sql = "DELETE FROM autoprov_mac_list WHERE id=" . $mac_id;
                sql($sql);
                $this->message['delete_line'] = "Deleted!";
                return(TRUE);
            } else {
    ...
    function delete_device($mac_id) {
        ...
        $sql = "DELETE FROM autoprov_line_list WHERE mac_id=" . $mac_id;
        sql($sql);
    ...
    function save_template($id, $custom, $variables) {
        ...
        if ($custom != "0") {
            $sql = "SELECT autoprov_model_list.max_lines, autoprov_product_list.config_files, autoprov_mac_list.*, autoprov_product_list.id as product_id, autoprov_product_list.long_name, autoprov_model_list.template_data, autoprov_product_list.cfg_dir, autoprov_brand_list.directory FROM autoprov_brand_list, autoprov_mac_list, autoprov_model_list, autoprov_product_list WHERE autoprov_mac_list.id=" . $id . " AND autoprov_mac_list.model = autoprov_model_list.id AND autoprov_model_list.brand = autoprov_brand_list.id AND autoprov_model_list.product_id = autoprov_product_list.id";
        } else {
            $sql = "SELECT autoprov_model_list.max_lines, autoprov_brand_list.directory, autoprov_product_list.cfg_dir, autoprov_product_list.config_files, autoprov_product_list.long_name, autoprov_model_list.template_data, autoprov_model_list.id as model_id, autoprov_template_list.* FROM autoprov_brand_list, autoprov_product_list, autoprov_model_list, autoprov_template_list WHERE autoprov_product_list.id = autoprov_template_list.product_id AND autoprov_brand_list.id = autoprov_product_list.brand AND autoprov_template_list.model_id = autoprov_model_list.id AND autoprov_template_list.id = " . $id;
        }
        ...
        if ($custom == "0") {
            $sql = 'UPDATE autoprov_template_list SET config_files_override = \'' . addslashes($config_files_selected) . '\', global_custom_cfg_data = \'' . addslashes($save) . '\' WHERE id =' . $id;
            $location = "template_manager";
        } else {
            $sql = 'UPDATE autoprov_mac_list SET config_files_override = \'' . addslashes($config_files_selected) . '\', template_id = 0, global_custom_cfg_data = \'' . addslashes($save) . '\' WHERE id =' . $id;
        ...
        if ($custom != 0) {
            ...
            $sql = 'SELECT id FROM autoprov_mac_list WHERE template_id = ' . $id;
