<?php
	/*
	$product_list = "SELECT * FROM autoprov_product_list WHERE id > 0";
	...
	*/
    ...
    //$endpoint->global_cfg = $endpoint->eda->sql("SELECT var_name, value FROM autoprov_global_vars",'getAssoc');	
?>
						...
						<div class="col-md-9">
							<div class="input-group">
                                ...
                                <?php
									...
									foreach (FreePBX::Autoprov()->listTZ("") as $row) {
										echo '<option data-icon="fa fa-clock-o" value="'.$row['value'].'" > '.$row['text'].'</option>';
									}
								?>
								</select>
      							<span class="input-group-btn">
        							<button class="btn btn-default" type="button" id='tzphp' onclick="epm_global_input_value_change_bt('#tz', sValue = '<?php echo FreePBX::Autoprov()->config->get('PHPTIMEZONE'); ?>');"><i class="fa fa-clock-o"></i> <?php echo _("TimeZone PBX")?></button>
      							</span>
    						</div>
						</div>
					</div>
