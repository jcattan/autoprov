 ...
 */
	...
	$family_list = FreePBX::Autoprov()->eda->all_products();
	$full_device_list = FreePBX::Autoprov()->eda->all_devices();
	$ava_exts = FreePBX::Autoprov()->display_registration_list();
...
//$endpoint->global_cfg['new'] = 1;
	...
	elseif(FreePBX::Autoprov()->configmod->get("srvip") == "") 
	{
		echo '<div class="alert alert-warning" role="alert">';
	...
	$i = 0;
	$list = array();
	$device_statuses = shell_exec(FreePBX::Autoprov()->configmod->get("asterisk_location")." -rx 'sip show peers'");
	...
	foreach($devices_list as $devices_row) {
		$line_list = FreePBX::Autoprov()->eda->get_lines_from_device($devices_row['id']);
		$list[$i] = $devices_row;
		$z = 0;
		if (($devices_row['template_id'] == 0) && (isset($devices_row['global_custom_cfg_data'])) ) {
			...
			$sql = "SELECT name FROM autoprov_template_list WHERE id =".$devices_row['template_id'];
	...
	$unknown_list = FreePBX::Autoprov()->eda->all_unknown_devices();
	
	foreach($unknown_list as $row) {	#Displays unknown phones in the database with edit and delete buttons
		$list[$i] = $row;
	
		$brand_info = FreePBX::Autoprov()->get_brand_from_mac($row['mac']);
...
$amp_send['AMPDBNAME'] = $amp_conf['AMPDBNAME'];
	...
	$sql = "SELECT DISTINCT autoprov_product_list.* FROM autoprov_product_list, autoprov_model_list WHERE autoprov_product_list.id = autoprov_model_list.product_id AND autoprov_model_list.hidden = 0 AND autoprov_model_list.enabled = 1 AND autoprov_product_list.hidden != 1 AND autoprov_product_list.cfg_dir !=  ''";
	...
	$sql = "SELECT DISTINCT autoprov_model_list.* FROM autoprov_product_list, autoprov_model_list WHERE autoprov_product_list.id = autoprov_model_list.product_id AND autoprov_model_list.hidden = 0 AND autoprov_model_list.enabled = 1 AND autoprov_product_list.hidden != 1 AND autoprov_product_list.cfg_dir !=  ''";
	$template_list = sql($sql, 'getAll', DB_FETCHMODE_ASSOC);
	$i = 1;
	$model_list = array();
	...
	<tbody>
		<tr>
			...
			<td align='center'>
				...
				<?php
				$brand_ava = FreePBX::Autoprov()->brands_available();
    		...
    		<td align='center'>
    			<label>  
    				<div id="demo">
            			...
            			<a href="#" onclick="return popitup('config.php?display=epm_config&amp;quietmode=1&amp;handler=file&amp;file=popup.html.php&amp;module=autoprov&amp;pop_type=edit_template&amp;edit_id={$edit_id}', 'Template Editor', '{$edit_id}')"><i class='icon-pencil'></i></a>
            		</div>
        		</label>
        	</td>
    		<td align='center'>
...
<a target="_blank" href="http://projects.colsolgrp.net/projects/show/autoprov"> CSG Software Projects.</a>
-->
...
$(function(){
	$("select#brand_edit").change(function(){
		...
		$.getJSON("config.php?type=tool&quietmode=1&handler=file&module=autoprov&file=ajax_select.html.php&atype=model",{id: $(this).val()}, function(j){
			var options = '';
...
$(function(){
	$("select#product_select").change(function(){
		$.ajaxSetup({ cache: false });
		$.getJSON("config.php?type=tool&quietmode=1&handler=file&module=autoprov&file=ajax_select.html.php&atype=template",{id: $(this).val()}, function(j){
			var options = '';
			for (var i = 0; i < j.length; i++) {
				options += '<option value="' + j[i].optionValue + '">' + j[i].optionDisplay + '</option>';
			}
...
$(function(){
	$("select#model_select").change(function(){
		$.ajaxSetup({ cache: false });
		$.getJSON("config.php?type=tool&quietmode=1&handler=file&module=autoprov&file=ajax_select.html.php&atype=mtemplate",{id: $(this).val()}, function(j){
			var options = '';
			for (var i = 0; i < j.length; i++) {
				options += '<option value="' + j[i].optionValue + '">' + j[i].optionDisplay + '</option>';
			}
...
$(function(){
	$("select#model_new").change(function(){
		$.ajaxSetup({ cache: false });
		$.getJSON("config.php?type=tool&quietmode=1&handler=file&module=autoprov&file=ajax_select.html.php&atype=template2",{id: $(this).val()}, function(j){
			var options = '';
			for (var i = 0; i < j.length; i++) {
				options += '<option value="' + j[i].optionValue + '">' + j[i].optionDisplay + '</option>';
			}
		...
		$.ajaxSetup({ cache: false });
		$.getJSON("config.php?type=tool&quietmode=1&handler=file&module=autoprov&file=ajax_select.html.php&atype=lines",{id: $(this).val()}, function(j){
			var options = '';
			for (var i = 0; i < j.length; i++) {
				options += '<option value="' + j[i].optionValue + '">' + j[i].optionDisplay + '</option>';
			}
...
function submit_stype(type,id) {
	newwindow=window.open('config.php?display=epm_config&quietmode=1&handler=file&file=popup.html.php&module=autoprov&pop_type=edit_specifics&edit_id=' + id + '&rand=' + new Date().getTime(),'name2','height=700,width=750,scrollbars=yes,location=no');
	if (window.focus) {newwindow.focus()}
	return false;
}
...
switch ($sub_type) {
        ...
        switch ($sub_type_sub) {
            ...
            case "button_edit":
                if(empty($_REQUEST['edit_id'])) {
                    ...
                    $sql = "UPDATE  autoprov_mac_list SET  model =  '".$_REQUEST['model_list']."' WHERE  id =".$_REQUEST['edit_id'];
            ...
            case "button_save":
                ...
                $sql = 'SELECT * FROM autoprov_line_list WHERE mac_id = '. $_REQUEST['edit_id'];
                ...
                foreach($lines_list as $row) {
                    ...
                    $sql = "UPDATE autoprov_line_list SET line = '".$_REQUEST['line_list_'.$row['luid']]."', ext = '".$_REQUEST['ext_list_'.$row['luid']]."', description = '".$endpoint->eda->escapeSimple($name)."' WHERE luid =  ". $row['luid'];
                    $endpoint->eda->sql($sql);
                }

                $sql = "UPDATE autoprov_mac_list SET template_id = '".$_REQUEST['template_list']."', model = '".$_REQUEST['model_list']."' WHERE id =  ". $_REQUEST['edit_id'];
                $endpoint->eda->sql($sql);
                ...
                break;
            case "delete":
                $sql = 'SELECT mac_id FROM autoprov_line_list WHERE luid = '.$_REQUEST['edit_id'] ;
    ...
    case "rebuild_configs_for_all_phones" :
        $sql = "SELECT autoprov_mac_list.id FROM autoprov_mac_list, autoprov_brand_list, autoprov_product_list, autoprov_model_list WHERE autoprov_brand_list.id = autoprov_product_list.brand AND autoprov_product_list.id = autoprov_model_list.product_id AND autoprov_mac_list.model = autoprov_model_list.id ORDER BY autoprov_product_list.cfg_dir ASC";
    ...
    case "reboot_brand" :
        if($_REQUEST['rb_brand'] != "") {
            $sql = 'SELECT autoprov_mac_list.id FROM autoprov_mac_list , autoprov_model_list , autoprov_brand_list , autoprov_product_list WHERE autoprov_brand_list.id = autoprov_model_list.brand AND autoprov_model_list.id = autoprov_mac_list.model AND autoprov_model_list.product_id = autoprov_product_list.id AND autoprov_brand_list.id = '.$_REQUEST['rb_brand'].' ORDER BY autoprov_product_list.cfg_dir ASC';
    ...
    case "go" :
        $sql = "UPDATE autoprov_global_vars SET value = '".$_REQUEST['netmask']."' WHERE var_name = 'nmap_search'";
        $endpoint->eda->sql($sql);
        ...
        foreach($temp as $key => $data) {
            if ((!$data['endpoint_managed']) AND ($data['brand'])) {
                $final[$key] = $data;
                $final[$key]['id'] = $key;
                $sqln = "SELECT * FROM autoprov_model_list WHERE enabled = 1 AND brand =".$data['brand_id'];
    ...
    case "change_brand" :
        if(isset($_REQUEST['selected'])) {
            if(($_REQUEST['brand_list_selected'] > 0) AND ($_REQUEST['model_list_selected'] > 0)) {
                foreach($_REQUEST['selected'] as $key => $data) {
                    $sql = "UPDATE autoprov_mac_list SET global_custom_cfg_data = '', template_id = 0, global_user_cfg_data = '', config_files_override = '', model = '".$_REQUEST['model_list_selected']."' WHERE id =  ". $_REQUEST['selected'][$key];
                    $endpoint->eda->sql($sql);
    ...
    case "rebuild_reboot" :
        if($_REQUEST['product_select'] == "") {
            ...
            $sql = "SELECT autoprov_mac_list.id FROM autoprov_mac_list, autoprov_brand_list, autoprov_product_list, autoprov_model_list WHERE autoprov_brand_list.id = autoprov_product_list.brand AND autoprov_product_list.id = autoprov_model_list.product_id AND autoprov_mac_list.model = autoprov_model_list.id AND autoprov_product_list.id = '".$_REQUEST['product_select']."'";
            $data = $endpoint->eda->sql($sql,'getAll',DB_FETCHMODE_ASSOC);
            foreach($data as $row) {
    ...
    case "mrebuild_reboot" :
        if($_REQUEST['model_select'] == "") {
            ...
            $sql = "SELECT autoprov_mac_list.id FROM autoprov_mac_list, autoprov_brand_list, autoprov_product_list, autoprov_model_list WHERE autoprov_brand_list.id = autoprov_product_list.brand AND autoprov_product_list.id = autoprov_model_list.product_id AND autoprov_mac_list.model = autoprov_model_list.id AND autoprov_model_list.id = '".$_REQUEST['model_select']."'";
            $data = $endpoint->eda->sql($sql,'getAll',DB_FETCHMODE_ASSOC);
            foreach($data as $row) {
