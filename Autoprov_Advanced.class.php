...
class Autoprov_Advanced
{
    ...
    public function ajaxHandler($module_tab = "", $command = "")
	{
		...
		elseif ($module_tab == "oui_manager") {
			...
			{
				case "oui":
					//$sql = 'SELECT autoprov_oui_list.id, autoprov_oui_list.oui , autoprov_brand_list.name, autoprov_oui_list.custom FROM autoprov_oui_list , autoprov_brand_list WHERE autoprov_oui_list.brand = autoprov_brand_list.id ORDER BY autoprov_oui_list.oui ASC';
					$sql = 'SELECT T1.id, T1.oui, T2.name, T1.custom FROM autoprov_oui_list as T1 , autoprov_brand_list as T2 WHERE T1.brand = T2.id ORDER BY T1.oui ASC';
	...
	private function epm_advanced_settings_saveconfig ()
	{
		...
		switch($dget['name']) {
			case "enable_ari":
				...
				$sql = "UPDATE autoprov_global_vars SET value='" . ($dget['value'] == "yes" ? "1": "0") . "' WHERE var_name='enable_ari'";
				break;

			case "enable_debug":
				$dget['value'] = strtolower($dget['value']);
				$sql = "UPDATE autoprov_global_vars SET value='" . ($dget['value'] == "yes" ? "1": "0") . "' WHERE var_name='debug'";
				break;

			case "disable_help":
				$dget['value'] = strtolower($dget['value']);
				$sql = "UPDATE autoprov_global_vars SET value='" . ($dget['value'] == "yes" ? "1": "0") . "' WHERE var_name='disable_help'";
				break;

			case "allow_dupext":
				$dget['value'] = strtolower($dget['value']);
				$sql = "UPDATE autoprov_global_vars SET value='" . ($dget['value'] == "yes" ? "1": "0") . "' WHERE var_name='show_all_registrations'";
				break;

			case "allow_hdfiles":
				$dget['value'] = strtolower($dget['value']);
				$sql = "UPDATE autoprov_global_vars SET value='" . ($dget['value'] == "yes" ? "1": "0") . "' WHERE var_name='allow_hdfiles'";
				break;

			case "tftp_check":
				$dget['value'] = strtolower($dget['value']);
				$sql = "UPDATE autoprov_global_vars SET value='" . ($dget['value'] == "yes" ? "1": "0") . "' WHERE var_name='tftp_check'";
				break;

			case "backup_check":
				$dget['value'] = strtolower($dget['value']);
				$sql = "UPDATE autoprov_global_vars SET value='" . ($dget['value'] == "yes" ? "1": "0") . "' WHERE var_name='backup_check'";
				break;
			...
			case "use_repo":
				...
				if (($dget['value'] == "yes") and (! $this->has_git())) {
					...
					$sql = "UPDATE autoprov_global_vars SET value='" . ($dget['value'] == "yes" ? "1": "0") . "' WHERE var_name='use_repo'";
				}
				break;
				...
				if ($dget['value'] != "") {
					if ((file_exists($dget['value'] = $dget['value'])) AND (is_dir($dget['value'] = $dget['value']))) {
						if (is_writable($dget['value'] = $dget['value'])) {
							$sql = "UPDATE autoprov_global_vars SET value='" . $dget['value'] . "' WHERE var_name='config_location'";
						} else {
			...
			case "srvip":
				$dget['value'] = trim($dget['value']);
				$sql = "UPDATE autoprov_global_vars SET value='" . $dget['value'] . "' WHERE var_name='srvip'";
				break;

			case "tz":
				$sql = "UPDATE autoprov_global_vars SET value='" . $dget['value'] . "' WHERE var_name='tz'";
				break;

			case "ntp_server":
				$dget['value'] = trim($dget['value']);
				$sql = "UPDATE autoprov_global_vars SET value='" . $dget['value'] . "' WHERE var_name='ntp'";
				break;

			case "nmap_loc":
				$sql = "UPDATE autoprov_global_vars SET value='" . $dget['value'] . "' WHERE var_name='nmap_location'";
				break;

			case "arp_loc":
				$sql = "UPDATE autoprov_global_vars SET value='" . $dget['value'] . "' WHERE var_name='arp_location'";
				break;

			case "asterisk_loc":
				$sql = "UPDATE autoprov_global_vars SET value='" . $dget['value'] . "' WHERE var_name='asterisk_location'";
				break;

			case "package_server":
				$sql = "UPDATE autoprov_global_vars SET value='" . $dget['value'] . "' WHERE var_name='update_server'";
				break;
			...
			case "cfg_type":
				...
				$sql = "UPDATE autoprov_global_vars SET value='" . $dget['value'] . "' WHERE var_name='server_type'";
				break;
	...
	{
		//$sql = 'SELECT * FROM autoprov_product_list WHERE hidden = 0 AND id > 0 ORDER BY long_name ASC';
		//$sql = 'SELECT * FROM autoprov_product_list WHERE hidden = 0 AND id > 0 AND brand IN (SELECT id FROM asterisk.autoprov_brand_list where hidden = 0) ORDER BY long_name ASC';
		$sql = 'SELECT * FROM autoprov_product_list WHERE hidden = 0 AND id IN (SELECT DISTINCT product_id FROM asterisk.autoprov_model_list where enabled = 1) AND brand IN (SELECT id FROM asterisk.autoprov_brand_list where hidden = 0) ORDER BY long_name ASC';
	...
	public function epm_advanced_poce_select()
	{
		...
		else
		{
			...
			$sql = 'SELECT * FROM `autoprov_product_list` WHERE `hidden` = 0 AND `id` = '.$dget['product_select'];
			$product_select_info = sql($sql, 'getRow', DB_FETCHMODE_ASSOC);

			$sql = "SELECT cfg_dir,directory,config_files FROM autoprov_product_list,autoprov_brand_list WHERE autoprov_product_list.brand = autoprov_brand_list.id AND autoprov_product_list.id ='" . $dget['product_select'] . "'";
			...
			$sql = "SELECT * FROM autoprov_custom_configs WHERE product_id = '" . $dget['product_select'] . "'";
			$data = sql($sql,'getAll', DB_FETCHMODE_ASSOC);
			$i = 0;
			if (count($data)) {
				//$data = sql($sql, 'getAll', DB_FETCHMODE_ASSOC);
			...
			$sql = "SELECT id, model FROM autoprov_model_list WHERE product_id = '" . $dget['product_select'] . "' AND enabled = 1 AND hidden = 0";
			$data = sql($sql, 'getAll', DB_FETCHMODE_ASSOC);
			$i = 1;
	...
	public function epm_advanced_poce_select_file()
	{
		...
		if ($dget['type_file'] == "sql") {
			$sql = 'SELECT * FROM autoprov_custom_configs WHERE id =' . $dget['file_id'];
			$row = sql($sql, 'getrow', DB_FETCHMODE_ASSOC);
		...
		elseif ($dget['type_file'] == "tfile")
		{
			if ($dget['file_id'] == "template_data_custom.xml")
			{
				...
				$sql = "SELECT * FROM autoprov_model_list WHERE id = '" . $dget['file_id'] . "'";
	...
	function epm_advanced_poce_save_file()
	{
		...
		if ($dget['type_file'] == "file") {
			...
			{
				$db = $this->db;
				$sql = 'INSERT INTO autoprov_custom_configs (name, original_name, product_id, data) VALUES (?,?,?,?)';
				$q = $db->prepare($sql);
		...
		elseif ($dget['type_file'] == "sql")
		{
			if ($dget['command'] == "poce_save_file")
			{
				$sql = "UPDATE autoprov_custom_configs SET data = '" . addslashes($dget['config_text']) . "' WHERE id = " . $dget['sendid'];
				sql($sql);
				$retarr = array("status" => true, "message" => "Saved to Database!");
			}
			elseif ($dget['command'] == "poce_save_as_file")
	...
	function epm_advanced_poce_delete_config_custom()
	{
		...
		if ($dget['type_file'] == "sql") {
			$sql = "DELETE FROM autoprov_custom_configs WHERE id =" . $dget['sql_select'];
			sql($sql);
			unset ($sql);
	...
	public function epm_advanced_manual_upload_export_brans_available()
	{
		...
		elseif ((! is_numeric($_REQUEST['package'])) OR ($_REQUEST['package'] < 0)) {
			...
			$sql = 'SELECT `name`, `directory` FROM `autoprov_brand_list` WHERE `id` = '.$dget['package'].'';
			$row = sql($sql, 'getRow', DB_FETCHMODE_ASSOC);
			...
			if ($row == "") {
				...
				//echo "Done! Click this link to download:<a href='modules/_ap_phone_modules/temp/export/".$row['directory']."-".$time.".tgz' target='_blank'>Here</a>";
			}
			unset ($dget);
		}
	}
	...
	public function epm_advanced_iedl_export($sFileName = "devices_list.csv")
	{
		...
		$sql = 'SELECT autoprov_mac_list.mac, autoprov_brand_list.name, autoprov_model_list.model, autoprov_line_list.ext,autoprov_line_list.line FROM autoprov_mac_list, autoprov_model_list, autoprov_brand_list, autoprov_line_list WHERE autoprov_line_list.mac_id = autoprov_mac_list.id AND autoprov_model_list.id = autoprov_mac_list.model AND autoprov_model_list.brand = autoprov_brand_list.id';
							...
							while (($device = fgetcsv($handle, filesize($uploadfile))) !== FALSE) {
								if ($device[0] != "") {
									if ($mac = $this->mac_check_clean($device[0])) {
										$sql = "SELECT id FROM autoprov_brand_list WHERE name LIKE '%" . $device[1] . "%' LIMIT 1";
										//$res = sql($sql);
										...
										//	$brand_id = $brand_id[0];
											...
											$sql_model = "SELECT id FROM autoprov_model_list WHERE brand = " . $brand_id . " AND model LIKE '%" . $device[2] . "%' LIMIT 1";
...
FreePBX::Autoprov()->add_device($mac, $model_id, $ext, 0, $line_id, $description);
	...
	private function epm_advanced_oui_remove()
	{
		...
		else
		{
			...
			$sql = "DELETE FROM autoprov_oui_list WHERE id = " . $dget['id'];
			sql($sql);
	...
	private function epm_advanced_oui_add()
	{
		...
		elseif ((! isset($_REQUEST['brand_new_oui'])) OR ($_REQUEST['brand_new_oui'] == "")) {
			...
			$sql = "INSERT INTO  autoprov_oui_list (oui, brand, custom) VALUES ('" . $dget['oui'] . "',  '" . $dget['brand'] . "',  '1')";
			sql($sql);
