...
class endpoint_htek_base extends endpoint_base {
    ...
    function create_encrypted_file($list) {
        ...
        foreach ($list as $key => $data) {
            ...
            if (file_exists("/usr/src/GS_CFG_GEN/bin/encode.sh")) {
                ...
                $params = $this->parse_hl_config($temporary_directory_location . "/" . $key);
                $contents = $this->hl_config_out($this->mac, $params);
            }
    ...
    function prepare_for_generateconfig() {
        //Hanlong likes lower case letters in its mac address
    ...
    function parse_hl_config($filename) {
    ...
    function hl_config_out($mac, $params) {
        $prev = 0;
