...
class Autoprov_Templates
{
	...
	public function epm_template_custom_config_get_global()
	{
		...
		else 
		{
			...
			if($dget['custom'] == 0) {
		        ...
		        $sql = 'SELECT global_settings_override FROM autoprov_template_list WHERE id = '.$dget['tid'];
			} else {
				//This is an individual template
		        $sql = 'SELECT global_settings_override FROM autoprov_mac_list WHERE id = '.$dget['tid'];;
			}
			$settings = sql($sql, 'getOne');

			if ((isset($settings)) and (strlen($settings) > 0)) {
				$settings = unserialize($settings);
				//$settings['tz'] = FreePBX::Autoprov()->listTZ(FreePBX::Autoprov()->configmod->get("tz"));
			} 
			else {
	...
	public function epm_template_custom_config_update_global ()
	{
		...
		else 
		{
			...
			if($dget['custom'] == 0) {
				//This is a group template
				$sql = "UPDATE autoprov_template_list SET global_settings_override = '".addslashes($settings_ser)."' WHERE id = ".$dget['tid'];
			} else {
				//This is an individual template
				$sql = "UPDATE autoprov_mac_list SET global_settings_override = '".addslashes($settings_ser)."' WHERE id = ".$dget['tid'];
			}
			unset($settings_ser);
			sql($sql);
	...
	public function epm_template_custom_config_reset_global()
	{
		...
		else 
		{
			...
			if($dget['custom'] == 0) {
				//This is a group template
				$sql = "UPDATE autoprov_template_list SET global_settings_override = NULL WHERE id = ".$dget['tid'];
			} else {
				//This is an individual template
				$sql = "UPDATE autoprov_mac_list SET global_settings_override = NULL WHERE id = ".$dget['tid'];
			}
			sql($sql);
	...
	public function epm_templates_del_template() 
	{
		...
		elseif ($_REQUEST['idsel'] <= 0) {
			...
			$sql = "DELETE FROM autoprov_template_list WHERE id = ". $dget['idsel'];
			sql($sql);
			$sql = "UPDATE autoprov_mac_list SET template_id = 0 WHERE template_id = ".$dget['idsel'];
			sql($sql);
	...
	public function epm_templates_add_template ()
	{
		...
		elseif ($_REQUEST['newclonemodel'] <= 0) {
			...
			$db = $this->db;
			$sql = "INSERT INTO autoprov_template_list (product_id, name, model_id) VALUES (?,?,?)";
			$q = $db->prepare($sql);
	...
	public function epm_templates_model_clone () 
	{
		...
		else
		{
			...
			$i=0;
			$out = array();
			$sql = "SELECT autoprov_model_list.id, autoprov_model_list.model as model FROM autoprov_model_list, autoprov_product_list WHERE autoprov_product_list.id = autoprov_model_list.product_id AND autoprov_model_list.enabled = 1 AND autoprov_model_list.hidden = 0 AND product_id = '". $dget['id']."'";
	...
	public function epm_templates_list_current_templates ()
	{
		...
		$sql = 'SELECT autoprov_template_list.*, autoprov_product_list.short_name as model_class, autoprov_model_list.model as model_clone, autoprov_model_list.enabled FROM autoprov_template_list, autoprov_model_list, autoprov_product_list WHERE autoprov_model_list.hidden = 0 AND autoprov_template_list.model_id = autoprov_model_list.id AND autoprov_template_list.product_id = autoprov_product_list.id';
	...
	{
    	if ($custom == 0) {
    		$sql = "SELECT model_id FROM autoprov_template_list WHERE id=" . $id;
    	} else {
    		$sql = "SELECT model FROM autoprov_mac_list WHERE id=" . $id;
    	}
    	...
    	$dReturn = array();
		if ($custom == 0) {
			$sql = "SELECT autoprov_model_list.max_lines, autoprov_model_list.model as model_name, autoprov_template_list.global_custom_cfg_data,  autoprov_product_list.config_files, autoprov_product_list.short_name, autoprov_product_list.id as product_id, autoprov_model_list.template_data, autoprov_model_list.id as model_id, autoprov_template_list.* FROM autoprov_product_list, autoprov_model_list, autoprov_template_list WHERE autoprov_product_list.id = autoprov_template_list.product_id AND autoprov_template_list.model_id = autoprov_model_list.id AND autoprov_template_list.id = " . $id;
		} else {
			$sql = "SELECT autoprov_model_list.max_lines, autoprov_model_list.model as model_name, autoprov_mac_list.global_custom_cfg_data, autoprov_product_list.config_files, autoprov_mac_list.*, autoprov_line_list.*, autoprov_model_list.id as model_id, autoprov_model_list.template_data, autoprov_product_list.id as product_id, autoprov_product_list.short_name, autoprov_product_list.cfg_dir, autoprov_brand_list.directory FROM autoprov_brand_list, autoprov_mac_list, autoprov_model_list, autoprov_product_list, autoprov_line_list WHERE autoprov_mac_list.id=" . $id . " AND autoprov_mac_list.id = autoprov_line_list.mac_id AND autoprov_mac_list.model = autoprov_model_list.id AND autoprov_model_list.brand = autoprov_brand_list.id AND autoprov_model_list.product_id = autoprov_product_list.id";
		}
		...
		foreach ($config_files_list as $files) 
		{
			...
			$sql = "SELECT * FROM  autoprov_custom_configs WHERE product_id = '" . $row['product_id'] . "' AND original_name = '" . $files . "'";
	...
	function areaAvailable($model, $area=NULL) {
    	$sql = "SELECT max_lines FROM autoprov_model_list WHERE id = '" . $model . "'";
    ...
    function generate_gui_html($cfg_data, $custom_cfg_data=NULL, $admin=FALSE, $user_cfg_data=NULL, $max_lines=3, $ext=NULL) {
    	...
    	//Check to see if there is a custom template for this phone already listed in the autoprov_mac_list database
    				...
    				if ($admin) {
    					...
    					switch ($type) {
    						case "loop_line_options":
    							...
    							$sql = "SELECT line FROM autoprov_line_list WHERE  ext = " . $ext;
