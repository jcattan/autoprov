...
function autoprov_configpageinit($pagename) {
    ...
    if ((($tech == 'sip') OR ($tech == 'pjsip')) AND (!empty($type))) {
        ...
        $doc_root = $amp_conf['AMPWEBROOT'] . "/admin/modules/autoprov/";
        if (file_exists($doc_root . "includes/functions.inc")) {
            ...
            if ($action == "del") {
                $sql = "SELECT mac_id,luid FROM autoprov_line_list WHERE ext = " . $extdisplay;
            ...
            if (($action == "edit") OR ($action == "add")) {
                ...
                if (!empty($mac)) {
                    ...
                    if ($endpoint->mac_check_clean($mac)) {
                        $sql = "SELECT id FROM autoprov_mac_list WHERE mac = '" . $endpoint->mac_check_clean($mac) . "'";
                        $macid = $endpoint->eda->sql($sql, 'getOne');
                        if ($macid) {
                            //In Database already

                            $sql = 'SELECT * FROM autoprov_line_list WHERE ext = ' . $extdisplay . ' AND mac_id = ' . $macid;
...
function autoprov_applyhooks() {
    global $currentcomponent;

    // Add the 'process' function - this gets called when the page is loaded, to hook into
    // displaying stuff on the page.
    $currentcomponent->addguifunc('autoprov_configpageload');
}

// This is called before the page is actually displayed, so we can use addguielem().
function autoprov_configpageload() {
    ...
    if (isset($tech) && (($tech == 'sip') OR ($tech == 'pjsip') OR ($tech == 'sip_generic'))) {
        ...
        if ($action != 'del') {
            ...
            $js = "
                        ...
                        $.getJSON(\"config.php?type=tool&quietmode=1&handler=file&module=autoprov&file=ajax_select.html.php&atype=model\",{id: value}, function(j){
                                var options = '';
            ...
            $sql = "SELECT mac_id,luid,line FROM autoprov_line_list WHERE ext = '" . $extdisplay . "' ";
            $line_info = $endpoint->eda->sql($sql, 'getRow', DB_FETCHMODE_ASSOC);
            if ($line_info) {

                $js = "
                        $.ajaxSetup({ cache: false });
                        $.getJSON('config.php?type=tool&quietmode=1&handler=file&module=autoprov&file=ajax_select.html.php&atype=template2',{id: value}, function(j){
                                var options = '';
                                for (var i = 0; i < j.length; i++) {
                                        options += '<option value=\"' + j[i].optionValue + '\">' + j[i].optionDisplay + '</option>';
                                }
                        ...
                        $.ajaxSetup({ cache: false });
                        $.getJSON('config.php?type=tool&quietmode=1&handler=file&module=autoprov&file=ajax_select.html.php&macid='+ macid +'&atype=lines',{id: value}, function(j){
                                var options = '';
                                for (var i = 0; i < j.length; i++) {
                                        options += '<option value=\"' + j[i].optionValue + '\">' + j[i].optionDisplay + '</option>';
                                }
...
//
                ...
                $js = "
                        ...
                        $.ajaxSetup({ cache: false });
                        $.getJSON('config.php?type=tool&quietmode=1&handler=file&module=autoprov&file=ajax_select.html.php&mac='+ mac +'&atype=lines',{id: value}, function(j){
                                var options = '';
                                for (var i = 0; i < j.length; i++) {
                                        options += '<option value=\"' + j[i].optionValue + '\">' + j[i].optionDisplay + '</option>';
                                }
...
function autoprov_hookProcess_core($viewing_itemid, $request) {

}

function autoprov_module_install_check_callback($mods = array()) {
